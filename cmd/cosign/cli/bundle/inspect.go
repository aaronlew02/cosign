...
func (c *InspectCmd) Exec() error {
	...
	if b.VerificationMaterial != nil {
		...
		if b.VerificationMaterial.GetCertificate() != nil {
			...
			certNode := vmNode.addChild("Content", "X.509 Certificate (v0.3)")
			populateCertificateSummary(certNode, cert.RawBytes)
		}

		if b.VerificationMaterial.GetX509CertificateChain() != nil {
			chain := b.VerificationMaterial.GetX509CertificateChain()
			chainNode := vmNode.addChild("Content", "X.509 Certificate Chain (v0.1/v0.2)")
			chainNode.addChild("Certificates", fmt.Sprintf("%d", len(chain.Certificates)))
		...
		if b.VerificationMaterial.GetPublicKey() != nil {
			...
			hint := b.VerificationMaterial.GetPublicKey().GetHint()
...
func populateCertificateSummary(n *node, rawBytes []byte) {
	...
	for _, ext := range cert.Extensions {
		...
		if readableName, ok := certExtensionMap[oidStr]; ok {
		...
		} else if strings.HasPrefix(oidStr, fulcioOidPrefix) {
			// Unknown Sigstore OID
			valStr := string(ext.Value)
			var decodedStr string
			_, err := asn1.Unmarshal(ext.Value, &decodedStr)
			if err == nil {
			...
			extsNode.addChild(fmt.Sprintf("OID: %s", oidStr), fmt.Sprintf("(Value: %s)", valStr))
		} else {
			...
			if err := sc.UnmarshalText([]byte(proof.Checkpoint.Envelope)); err == nil {
				...
				if len(sc.Signatures) > 0 {
					...
					for _, sig := range sc.Signatures {
						...
						witnessesNode.addChild(sig.Name, fmt.Sprintf("Signature Present (%d bytes)", len(sigBytes)))
					}
				}
			} else {
...
func populateContentSummary(n *node, b *protobundle.Bundle) {
	...
	case *protobundle.Bundle_DsseEnvelope:
		...
		for i, sig := range content.DsseEnvelope.Signatures {
			...
			if sig.Keyid != "" {
				sigNode.addChild("KeyID", sig.Keyid)
			}
		}
...
const fulcioOidPrefix = "1.3.6.1.4.1.57264.1."
